...
<main id="content" role="main" class="main font-public-sans">
    <div class="content container-fluid">
                ...
                <div class="col-sm mb-2 mb-sm-0">
                    <nav aria-label="breadcrumb">
                        <ol class="breadcrumb breadcrumb-no-gutter">
                            ...
                            <li class="breadcrumb-item active">Modifier le Type</li>
                        </ol>
                    </nav>
                    <h2 class="mb-0 text-secondary-custom">Modifier le Type de Mouvement</h2>
                    <p class="mb-0 text-muted">Modifiez les informations de ce type de mouvement</p>
                </div>
        ...
        <div id="loadingContainer" class="text-center py-5">
            ...
            <p class="mt-3 text-muted">Chargement des informations du type...</p>
        </div>

        <!-- Form -->
        <form id="movementTypeForm" class="form-container fade-in" style="display: none;">
            ...
            <div class="form-section">
                ...
                <div class="form-group-modern">
                    <label class="form-label-modern">Statut d'activation</label>
                    <div class="status-toggle">
                        <div class="toggle-switch" id="statusToggle" onclick="toggleStatus()"></div>
                        <div class="status-info">
                            <div class="status-label" id="statusLabel">Type Inactif</div>
                            <div class="status-description" id="statusDescription">
                                Ce type de mouvement ne sera pas disponible pour les nouveaux mouvements.
                            </div>
                        </div>
                    </div>
                    <input type="hidden" id="is_active" name="is_active" value="false">
                    <div class="form-help">
                        <i class="bi-info-circle"></i>
                        Un type inactif ne peut pas être utilisé pour créer de nouveaux mouvements de stock.
                    </div>
                </div>
            </div>
            ...
            <div class="form-section">
                <div class="row">
                    ...
                    <div class="col-md-4">
                        <button type="button" class="btn btn-warning-modern w-100" onclick="deleteMovementType()">
...
<div id="loadingOverlay" class="loading-overlay">
    ...
    <div class="loading-text">Sauvegarde en cours...</div>
</div>

<script>
    let movementTypeId = null;
    let movementTypeData = null;

    document.addEventListener('DOMContentLoaded', function() {
        movementTypeId = getMovementTypeIdFromUrl();
        if (movementTypeId) {
            loadMovementType();
            setupEventListeners();
        } else {
            showError('ID du type de mouvement non trouvé dans l\'URL');
        }
    });

    function getMovementTypeIdFromUrl() {
        const path = window.location.pathname;
        const matches = path.match(/\/stock\/type-mouvement\/([^\/]+)\/edit/);
    ...
    async function loadMovementType() {
        try {
            const response = await fetch(`https://toure.gestiem.com/api/stock-movement-types/${movementTypeId}`, {
                headers: {
            ...
            if (response.ok) {
                ...
                movementTypeData = result.data;
                populateForm();
                
                document.getElementById('loadingContainer').style.display = 'none';
                document.getElementById('movementTypeForm').style.display = 'block';
            } else {
                const errorData = await response.json();
                showError(errorData.message || 'Erreur lors du chargement du type de mouvement');
            }
        } catch (error) {
    ...
    function populateForm() {
        if (!movementTypeData) return;

        // Remplir les champs
        document.getElementById('name').value = movementTypeData.name || '';
        document.getElementById('description').value = movementTypeData.description || '';
        
        // Sélectionner la direction
        const direction = movementTypeData.direction || movementTypeData.movement_type;
        if (direction) {
            document.getElementById('direction').value = direction;
            const directionOption = document.querySelector(`[data-direction="${direction}"]`);
            if (directionOption) {
        ...
        const isActive = movementTypeData.is_active !== false;
        document.getElementById('is_active').value = isActive ? 'true' : 'false';
        updateStatusToggle(isActive);
    }
    ...
    function toggleStatus() {
        ...
        document.getElementById('is_active').value = newStatus ? 'true' : 'false';
    }
    ...
    function updateStatusToggle(isActive) {
        ...
        if (isActive) {
            ...
            label.textContent = 'Type Actif';
            description.textContent = 'Ce type de mouvement est disponible pour créer de nouveaux mouvements.';
        } else {
            toggle.classList.remove('active');
            label.textContent = 'Type Inactif';
            description.textContent = 'Ce type de mouvement ne sera pas disponible pour les nouveaux mouvements.';
        }
    }
    ...
    async function handleSubmit(e) {
        ...
        try {
            ...
            if (response.ok) {
                ...
                showAlert('✓ Type de mouvement mis à jour avec succès !', 'success');
    ...
    async function deleteMovementType() {
        if (!movementTypeData) return;

        const confirmMessage = `Êtes-vous sûr de vouloir supprimer le type "${movementTypeData.name}" ?\n\nCette action est irréversible et supprimera définitivement ce type de mouvement.`;
        ...
        try {
            ...
            if (response.ok) {
                showAlert('✓ Type de mouvement supprimé avec succès !', 'success');
                ...
                const errorData = await response.json();
                showAlert(`Erreur: ${errorData.message || 'Erreur lors de la suppression'}`, 'danger');
            }
    ...
    function showError(message) {
        document.getElementById('loadingContainer').innerHTML = `
            ...
            <button class="btn btn-primary-custom" onclick="window.location.href='/stock/types-mouvements'">
