...
<style>
    ...
    .modify-chauffeur-header::before {
        content: '';
        position: absolute;
        top: 0;
        left: 0;
...
<div class="modify-chauffeur-wrapper font-public-sans">
    <!-- Header -->
    <div class="modify-chauffeur-header fade-in">
        <div class="d-flex justify-content-between align-items-center">
            <h1 class="modify-chauffeur-title">
                <div class="modify-chauffeur-icon">
                    <i class="bi bi-pencil-square"></i>
                </div>
                Modifier le Chauffeur
            </h1>
            <a href="/chauffeur/<?php echo $chauffeur_id; ?>" class="btn-modern btn-outline-modern">
                <i class="bi bi-arrow-left"></i> Retour
            </a>
        </div>
        <p class="text-muted mb-0">Modifiez les informations du chauffeur</p>
    </div>
    ...
    <!-- Form -->
    <form id="modifyChauffeurForm" class="form-container fade-in" style="display: none;">
        ...
        <div class="form-actions">
            ...
            <button type="button" id="submitModifyChauffeur" class="btn-modern btn-warning-modern">
...
<script>
    ...
    async function loadChauffeurData() {
        try {
            ...
            if (response.ok) {
                ...
                console.log('Données du chauffeur:', result);
                
                chauffeurData = result.data;
                populateForm();
                
                document.getElementById('loadingData').style.display = 'none';
                document.getElementById('modifyChauffeurForm').style.display = 'block';
            } else {
    ...
    function populateForm() {
        ...
        document.getElementById('name').value = chauffeurData.name || '';
        document.getElementById('phone').value = chauffeurData.phone || '';
        document.getElementById('numeroPermis').value = chauffeurData.numero_permis || '';
        document.getElementById('dateExpirationPermis').value = chauffeurData.date_expiration_permis || '';
        document.getElementById('status').value = chauffeurData.status || 'actif';
    }

    function setupEventListeners() {
        // Soumission du formulaire
        const form = document.getElementById('modifyChauffeurForm');
        if (form) {
            form.addEventListener('submit', handleSubmit);
        }

        // Event listener pour le bouton de soumission
        const submitButton = document.getElementById('submitModifyChauffeur');
        if (submitButton) {
    ...
    async function handleSubmit(e) {
        ...
        try {
            ...
            if (response.ok) {
                ...
                showAlert('✓ Chauffeur mis à jour avec succès !', 'success');
                
                // Rediriger vers les détails après 2 secondes
                setTimeout(() => {
                    window.location.href = `/chauffeur/${chauffeurId}?updated=1`;
                }, 2000);
            } else {
                const errorResult = await response.json();
